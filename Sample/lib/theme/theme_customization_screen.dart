...
class _ThemeCustomizationScreenState extends State<ThemeCustomizationScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Customize your Theme')),
      body: Column(
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        _colorOptionItem(state.theme?.sdkBackgroundColor?.color, "SDK background", (
                          ...
                          final newTheme = (state.theme ?? ch.CheckoutTheme()).copyWith(
                            sdkBackgroundColor: ch.ColorState(color: color),
                          );
                          context.read<ThemeCustomizationScreenCubit>().onThemeChanged(newTheme);
                        }),
                        const SizedBox(height: dividerPadding),
                        ...
                        const SizedBox(height: dividerPadding),
                        _colorOptionItem(
                          state.theme?.modalBackgroundColor?.color,
                          "Modal background",
                          (color) {
                            final newTheme = (state.theme ?? ch.CheckoutTheme()).copyWith(
                              modalBackgroundColor: ch.ColorState(color: color),
                            );
                            context.read<ThemeCustomizationScreenCubit>().onThemeChanged(newTheme);
                          },
                        ),
                        ...
                        _colorOptionItem(
                          ...
                          (color) {
                            final newTheme = (state.theme ?? ch.CheckoutTheme()).copyWith(
                              selectPaymentMethodHeaderBackgroundColor: ch.ColorState(color: color),
                            );
                            context.read<ThemeCustomizationScreenCubit>().onThemeChanged(newTheme);
                          },
                        ),
                        ...
                        const SizedBox(height: dividerPadding),
                        _colorOptionItem(
                          state.theme?.savePhoneNumberIconColor?.color,
                          "Save Phone Number Icon Color",
                          (color) {
                            final newTheme = (state.theme ?? ch.CheckoutTheme()).copyWith(
                              savePhoneNumberIconColor: ch.ColorState(color: color),
                            );
                            context.read<ThemeCustomizationScreenCubit>().onThemeChanged(newTheme);
                          },
                        ),
                        ...
                        const SizedBox(height: dividerPadding),
                        _colorOptionItem(
                          state.theme?.selectorIconColor?.color,
                          "Select Payment Icon Color",
                          (color) {
                            final newTheme = (state.theme ?? ch.CheckoutTheme()).copyWith(
                              selectorIconColor: ch.ColorState(color: color),
                            );
                            context.read<ThemeCustomizationScreenCubit>().onThemeChanged(newTheme);
                          },
                        ),
                        ...
                        const SizedBox(height: dividerPadding),
                        _colorOptionItem(
                          state.theme?.paymentItemBackgroundColor?.color,
                          "Select Payment Background Color",
                          (color) {
                            final newTheme = (state.theme ?? ch.CheckoutTheme()).copyWith(
                              paymentItemBackgroundColor: ch.ColorState(color: color),
                            );
                            context.read<ThemeCustomizationScreenCubit>().onThemeChanged(newTheme);
                          },
                        ),
  ...
  InkWell _textOptionItem(String title, ch.TextStyle? textStyle, Function(ch.TextStyle) onChange) {
    final ValueNotifier<String?> initialFontFamily = ValueNotifier(textStyle?.fontFamily);
    final ValueNotifier<Color?> initialColor = ValueNotifier(textStyle?.textColor?.color);
    final ValueNotifier<int?> initialFontSize = ValueNotifier(textStyle?.fontSize);
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return _textEditDialog(
              initialFontFamily: initialFontFamily.value,
              initialColor: initialColor.value,
              fontSize: initialFontSize.value,
              onChanged: (fontFamily, color, fontSize) async {
                initialFontFamily.value = fontFamily;
  ...
  InkWell _colorOptionItem(Color? colorStyle, String title, Function(Color) onChange) {
  ...
  Widget _expandableOptionItem({required String title, required Widget child}) {
    final AnimationController rotationController = AnimationController(
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, btnValue, __) => Column(
        children: [
          InkWell(
            child: Padding(
          ...
          expanded.value
              ? Padding(padding: const EdgeInsets.only(left: 14.0), child: child)
              : SizedBox.shrink(),
        ],
      ),
    );
      ...
      children: [
        ...
        ValueListenableBuilder(
          ...
          builder: (context, btnValue, __) => _colorOptionItem(
            btnValue?.rippleColor?.colorDisabled,
            "Button Disabled Color",
            (color) {
              final button = (btnValue ?? ch.ButtonComponent());
              btn.value = button.copyWith(
                rippleColor: (button.rippleColor ?? ch.RippleColor()).copyWith(
                  colorDisabled: color,
                ),
              );
            },
  ...
  Widget _inputFieldModifier(
    ch.TextFieldStyle? inputField,
    Function(ch.TextFieldStyle inputField) onChange,
  ) {
  ...
  Widget _switchModifier(
    ch.SwitchComponent? switchComponent,
    Function(ch.SwitchComponent switchComponent) onChange,
  ) {
      ...
      children: [
        ...
        ValueListenableBuilder(
          ...
          builder: (context, value, __) => _colorOptionItem(
            value?.checkedTrackDecorationColor,
            "Checked Track Decoration",
            (color) {
              btn.value = (value ?? ch.SwitchComponent()).copyWith(
                checkedTrackDecorationColor: color,
              );
            },
          ),
        ),
        ...
        ValueListenableBuilder(
          valueListenable: btn,
          builder: (context, value, __) => _colorOptionItem(
            value?.uncheckedTrackDecorationColor,
            "Unchecked Track Decoration",
            (color) {
              btn.value = (value ?? ch.SwitchComponent()).copyWith(
                uncheckedTrackDecorationColor: color,
              );
            },
          ),
        ),
  ...
  Widget _textEditDialog({
    String title = "Pick a color!",
    required String? initialFontFamily,
    required Color? initialColor,
    required int? fontSize,
    required Function(String?, Color?, int?) onChanged,
  }) {
    ...
    final ValueNotifier<int?> editedFontSize = ValueNotifier(fontSize);
    return AlertDialog(
      title: Text(title),
      content: SingleChildScrollView(
        child: Column(
      ...
      actions: <Widget>[
        ...
        ElevatedButton(
          ...
          onPressed: () {
            Navigator.of(context).pop();
            onChanged(selectedFontFamily.value, _localColor.value, editedFontSize.value);
          },
        ),
      ],
    );
